...
- Exit auto-backup: auto export Excel on page exit to reduce data-loss risk after browser cache cleanup
...
### 3.3 Exit Auto-Backup (data-loss guard)

- Click `Auto Backup` in the top header.
- Choose `1`: bind a local Excel file and enable auto backup (recommended).
  - After first binding, the app overwrites the same file on page exit.
- Choose `2`: enable download backup mode (fallback).
  - On page exit, the app triggers a timestamped Excel download.
- Choose `3`: disable auto backup.
- Choose `4`: run one backup immediately.

Notes:

- Direct file overwrite requires File System Access API (typically available in modern Chrome/Edge).
- If unsupported or permission is denied, the app automatically falls back to download mode.
...
- Manual CLI deploy from local repo: `quick-deploy-vercel.bat` (Windows) or `sh quick-deploy-vercel.sh` (Linux/macOS).
- GitHub Actions deploy: run workflow `.github/workflows/vercel-deploy.yml` via `workflow_dispatch` and set `VERCEL_TOKEN` in repository secrets.
