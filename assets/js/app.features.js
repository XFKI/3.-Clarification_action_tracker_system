...
function renderOverview(){
  ...
  const closure=buildClosureQualitySnapshot();
  const ownerWeekLoad=buildOwnerWeekLoad().slice(0,8);
  ...
  <div class="charts-row" style="align-items:stretch">
    <div class="chart-box" style="min-height:0"><h3>${t('闭环效率快照','Closure Quality Snapshot')}</h3><div class="storage-grid" style="grid-template-columns:repeat(4,minmax(120px,1fr));margin-bottom:8px"><div class="storage-stat"><div class="storage-k">${t('已关闭总数','Closed')}</div><div class="storage-v">${closure.closed}</div></div><div class="storage-stat"><div class="storage-k">${t('7天内关闭','Closed <= 7d')}</div><div class="storage-v">${closure.within7}</div></div><div class="storage-stat"><div class="storage-k">${t('7天达成率','<=7d Rate')}</div><div class="storage-v">${closure.within7Rate}%</div></div><div class="storage-stat"><div class="storage-k">${t('平均关闭天数','Avg Close Days')}</div><div class="storage-v">${closure.avgCloseDays}</div></div></div><div class="storage-note">${t('口径：仅统计技术澄清与会议纪要，按创建时间到关闭时间计算。','Scope: clarification + meeting only, measured from created to closed date.')}</div></div>
    <div class="chart-box" style="min-height:0"><h3>${t('到期周负荷 Top8','Due-Week Workload Top8')}</h3><div class="table-scroll" style="max-height:220px"><table><thead><tr><th>${t('责任方','Owner')}</th><th>${t('到期周','Due Week')}</th><th>${t('开放项','Open Items')}</th><th>${t('动作','Action')}</th></tr></thead><tbody>${ownerWeekLoad.length?ownerWeekLoad.map(r=>`<tr><td>${escHtml(r.owner)}</td><td class="cell-date">${escHtml(r.week)}</td><td>${r.count}</td><td><button class="btn btn-outline" style="padding:2px 8px;font-size:.68rem" onclick="applyOwnerWeekPreset('${escJs(r.owner)}','${escJs(r.week)}')">${t('查看明细','Open Details')}</button></td></tr>`).join(''):`<tr><td colspan="4" class="no-data">${t('暂无负荷数据','No workload data')}</td></tr>`}</tbody></table></div></div>
  </div>
  `;
}
